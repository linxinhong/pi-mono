...
const CHANNELS_SUBDIR = "chat";  // 频道数据子目录
...
function parseArgs(): ParsedArgs {
	...
	for (let i = 0; i < args.length; i++) {
		...
		if (arg.startsWith("--sandbox=")) {
			...
			console.error(JSON.stringify({ feishu: { appId: "cli_xxx", appSecret: "xxx", port: 3000, model: "groq/qwen-qwq-32b" } }, null, 2));
			console.error("");
...
const workspaceDir = (parsedArgs.dataDir !== DEFAULT_WORKSPACE_DIR)
	? parsedArgs.dataDir
	: (feishuConfig?.dataDir || DEFAULT_WORKSPACE_DIR);

// Ensure directories exist
if (!existsSync(workspaceDir)) {
	mkdirSync(workspaceDir, { recursive: true });
}
const chatDir = join(workspaceDir, CHANNELS_SUBDIR);
...
function createFeishuContext(event: FeishuEvent, feishu: FeishuBot, state: ChannelState, isEvent?: boolean) {
	...
	return {
		...
		replaceMessage: async (text: string) => {
			// Send final response as new message (status message keeps showing "completed")
			responseMessageId = await feishu.postMessage(event.channel, text);
...
const sharedStore = new ChannelStore({ chatDir, appId: FEISHU_APP_ID!, appSecret: FEISHU_APP_SECRET! });
