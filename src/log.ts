...
function formatContext(ctx: LogContext): string {
	if (ctx.channelId.startsWith("ou_") || ctx.channelId.startsWith("on_")) {
...
export function logToolError(ctx: LogContext, toolName: string, durationMs: number, error: string): void {
	const duration = (durationMs / 1000).toFixed(1);
	console.log(chalk.yellow(`${timestamp()} ${formatContext(ctx)} X ${toolName} (${duration}s)`));
...
export function logDownloadStart(ctx: LogContext, filename: string, localPath: string): void {
	console.log(chalk.yellow(`${timestamp()} ${formatContext(ctx)} v Downloading attachment`));
...
export function logDownloadError(ctx: LogContext, filename: string, error: string): void {
	console.log(chalk.yellow(`${timestamp()} ${formatContext(ctx)} X Download failed`));
...
export function logAgentError(ctx: LogContext | "system", error: string): void {
	...
	console.log(chalk.yellow(`${timestamp()} ${context} X Agent error`));
...
export function logStartup(workingDir: string, sandbox: string): void {
	console.log("Starting pi-feishu bot...");
	console.log(`  Working directory: ${workingDir}`);
	console.log(`  Sandbox: ${sandbox}`);
}

export function logConnected(): void {
	console.log("pi-feishu bot connected and listening!");
	console.log("");
}

export function logDisconnected(): void {
	console.log("pi-feishu bot disconnected.");
}
