...
import type { FeishuBot, FeishuEvent } from "./feishu.js";
...
export type FeishuScheduledEvent = ImmediateEvent | OneShotEvent | PeriodicEvent;
...
export class EventsWatcher {
	...
	private async handleFile(filename: string): Promise<void> {
		...
		let event: FeishuScheduledEvent | null = null;
	...
	private parseEvent(content: string, filename: string): FeishuScheduledEvent | null {
	...
	private execute(filename: string, event: FeishuScheduledEvent, deleteAfter: boolean = true): void {
		...
		const syntheticEvent: FeishuEvent = {
			type: "mention",
		...
		const enqueued = this.feishu.enqueueEvent(syntheticEvent);
 ...
 */
export function createEventsWatcher(workspaceDir: string, feishu: FeishuBot): EventsWatcher {
	const eventsDir = join(workspaceDir, "events");
	return new EventsWatcher(eventsDir, feishu);
}
