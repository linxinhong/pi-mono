...
export interface CardElement {
	...
	header?: { title: { content: string; tag: string } };
}
...
export class FeishuBot {
	...
	private async handleMessageEvent(event: any): Promise<void> {
		...
		try {
			...
			if (msgType === "text") {
				...
				files = [{ name: parsedContent.file_name, file_key: parsedContent.file_key, file_token: parsedContent.file_token }];
		...
		const isMention = text.includes(`@${this.botUserId}`) || content.includes(`"at_user_id"`);
	...
	private extractTextFromPost(postContent: any): string {
		...
		if (Array.isArray(postContent.content)) {
			return postContent.content.map((block: any) => {
