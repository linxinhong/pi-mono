...
import { getModel, type ImageContent, type Model, type Api } from "@mariozechner/pi-ai";
import {
	AgentSession,
	AuthStorage,
	convertToLlm,
...
function getMemory(channelDir: string): string {
	...
	// channelDir = /workspace/chat/oc_xxx, workspace = /workspace
	// 所以需要往上一层才能到达 workspace 根目录
...
function createRunner(sandboxConfig: SandboxConfig, channelId: string, channelDir: string): AgentRunner {
	...
	// channelDir = /workspace/chat/oc_xxx, 需要获取 /workspace
	const workspacePath = executor.getWorkspacePath(join(channelDir, "..", ".."));

	const tools = createFeishuTools(executor);
...
function translateToHostPath(
	...
	if (workspacePath === "/workspace") {
		// 容器内: /workspace/chat/oc_xxx/... -> 主机: channelDir/...
		const chatChannelPrefix = `/workspace/chat/${channelId}/`;
		if (containerPath.startsWith(chatChannelPrefix)) {
			return join(channelDir, containerPath.slice(chatChannelPrefix.length));
		}
