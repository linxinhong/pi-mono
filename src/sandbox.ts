...
export function parseSandboxArg(value: string): SandboxConfig {
	...
	if (value.startsWith("docker:")) {
		...
		if (!container) {
			console.error("Error: docker sandbox requires container name (e.g., docker:feishu-sandbox)");
			process.exit(1);
		}
...
export async function validateSandbox(config: SandboxConfig): Promise<void> {
	...
	try {
		...
		console.error("Create it with: docker run -d --name feishu-sandbox alpine:latest tail -f /dev/null");
		process.exit(1);
	}
