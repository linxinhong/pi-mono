...
export interface ChannelStoreConfig {
	chatDir: string;  // 频道数据目录 (e.g., ~/.pi/feishu/chat)
	appId: string;
	appSecret: string;
}
...
export class ChannelStore {
	...
	getChannelDir(channelId: string): string {
		const dir = join(this.chatDir, channelId);
		if (!existsSync(dir)) {
	...
	getLastTimestamp(channelId: string): string | null {
		const logPath = join(this.chatDir, channelId, "log.jsonl");
	...
	private async downloadAttachment(localPath: string, fileKey: string): Promise<void> {
		const filePath = join(this.chatDir, localPath);

		const dir = join(this.chatDir, localPath.substring(0, localPath.lastIndexOf("/")));
		if (!existsSync(dir)) {
			mkdirSync(dir, { recursive: true });
		}
