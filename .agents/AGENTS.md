# pi-mono Development Rules

## Project Structure & Module Organization

pi-mono is a monorepo containing multiple packages for building AI coding agents:

### Core Packages

| Package | Description | Key Files |
|---------|-------------|-----------|
| `packages/ai` | Multi-provider LLM API abstraction | `src/stream.ts`, `src/providers/` |
| `packages/agent` | Agent runtime and tool execution | `src/agent.ts`, `src/agent-loop.ts` |
| `packages/coding-agent` | Interactive coding agent CLI | `src/cli/`, `src/core/` |
| `packages/tui` | Terminal UI components | `src/` |
...
- E2E tests use `*.e2e.test.ts` naming
...
- If dependencies are missing (e.g., `node_modules` missing, `vitest not found`), run `npm install` first
- If TypeScript errors occur, check that you're using the correct import patterns (no inline imports)
- For test failures, run specific test files to isolate issues
- When in doubt, run `npm run check` to see all current issues

## GitHub Search (`gh`)

- Prefer targeted keyword search before proposing new work or duplicating fixes
- Use `--repo` + `--match title,body` first; add `--match comments` when triaging follow-up threads
- PRs: `gh search prs --repo <owner>/<repo> --match title,body --limit 50 -- "keyword"`
- Issues: `gh search issues --repo <owner>/<repo> --match title,body --limit 50 -- "keyword"`
- Structured output: `gh search issues --repo <owner>/<repo> --match title,body --limit 50 --json number,title,state,url,updatedAt -- "keyword" --jq '.[] | "\(.number) | \(.state) | \(.title)"'`
